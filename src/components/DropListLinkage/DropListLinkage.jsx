...
class DropListLinkage extends React.PureComponent {
  ...
  get selectNodes() {
    ...
    return this.selectItems.map((colItem, colIdx) => {
      const selectItem = colItem.children.map((rowItem, rowIdx) => {
        ...
        collapseSelectTree = isFinalLevel
      ...
      return (
        <div
          className={classNames('select-item', styles.selectItem, {
  ...
  /** 处理 document 点击 */
  enableDocumentEvent = () => {
    DOM.getElement().removeEventListener('click', this.handleDocument)
    DOM.getElement().addEventListener('click', this.handleDocument)
  }

  handleDocument = ev => {
    if (oSelect instanceof HTMLElement) {
      // eslint-disable-next-line
      const selectItems = Array.from(
        oSelect.getElementsByClassName('select-item')
      )
      if (!selectItems.some(item => item.contains(ev.target))) {
        // 点击了 select tree 之外的地方
        this.setState(() => ({ select: cloneDeep($selectTree) }))
        // 关闭情况，清除 document click event
        DOM.getElement().removeEventListener('click', this.handleDocument)
      }
    }

    if (collapseSelectTree) {
      // 关闭情况，清除 document click event
      DOM.getElement().removeEventListener('click', this.handleDocument)
    }
  }
